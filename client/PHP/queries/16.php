<?php
   ...
   $conn = require('../dbConnection.php');
    ...
    if($_POST){
        ...
        $sql = "
        SELECT dnw.fName, dnw.lName, dnw.firstDate, dnw.role, dnw.dateOfBirth, dnw.emailAddress, SUM(HOUR(TIMEDIFF(s.endTime, s.startTime))) as totalHours
        FROM
           ...
           FROM 
               ...
               WHERE medicareNb  IN
              (SELECT medicareNb FROM nbc353_4.IsInfected WHERE nature = 'COVID-19' GROUP BY medicareNb
             HAVING COUNT(nbc353_4.IsInfected.dateOfInfection) >= 3)
                     ) 
               UNION
               (SELECT * 
               ...
               WHERE medicareNb IN
               (SELECT medicareNb FROM  nbc353_4.IsInfected WHERE nature = 'COVID-19' GROUP BY medicareNb
             HAVING COUNT(nbc353_4.IsInfected.dateOfInfection) >= 3))) as dn
             , nbc353_4.WorksIn as w
